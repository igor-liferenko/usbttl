...
@*4 Stream functions for EEPROM source/destination data.
...
\param[out] Buffer          Pointer to the destination data buffer to write to, located in
 EEPROM memory space.
\param[in]  Length          Number of bytes to send via the currently selected endpoint.
\param[in]  BytesProcessed  Pointer to a location where the total number of bytes processed
 in the current
...
\param[in] Buffer          Pointer to the source data buffer to read from.
\param[in] Length          Number of bytes to read for the currently selected endpoint into the
 buffer.
\param[in] BytesProcessed  Pointer to a location where the total number of bytes processed
 in the current
         transaction should be updated, \c NULL if the entire stream should be written at once.
